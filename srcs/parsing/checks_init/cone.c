...
/*   Updated: 2024/06/03 14:04:33 by lcamerly         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    check_cone(char *s)
{
    ...
    if (ft_atof(tmp[3]) < 0 || ft_atof(tmp[3]) > 90)
        exit_error("Cone angle must be positive and inferior to 180 degrees!\nExiting...\n");
    if (ft_atof(tmp[4]) < 0)
...
void init_cone(char *s, t_minirt *minirt)
{
	...
	if (!tmp2)
		exit_error("Error\nMalloc failed in cone.c:66\nExiting...\n");
	o = create_obj(cone, c, (t_vec3){ft_atof(*tmp2) / 255, ft_atof(*(tmp2 + 1)) / 255, ft_atof(*(tmp2 + 2)) / 255});
