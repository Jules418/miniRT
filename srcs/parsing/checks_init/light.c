...
/*   Updated: 2024/05/29 00:45:36 by lcamerly         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void init_light(char *s, t_minirt *minirt)
{
    ...
    minirt->scene->light.pos = (t_vec3){ft_atof(*tmp2), ft_atof(*(tmp2 + 1)), ft_atof(*(tmp2 + 2))};
    minirt->scene->light.brightness = ft_atof(tmp[2]);
    tmp2 = gc_split(tmp[3], ',');
    if (!tmp2)
        exit_error("Error\nMalloc failed in camera.c:63\nExiting...\n");
    minirt->scene->light.color = (t_vec3){ft_atof(*tmp2) / 255, ft_atof(*(tmp2 + 1)) / 255, ft_atof(*(tmp2 + 2)) / 255};
}